...
class TestTokenizeOperatorsAndParens(unittest.TestCase):
    ...
    def test_star_star_star_is_power_then_star(self):
        ...
        tokens = tokenize("***")
        self.assertEqual([t.type for t in tokens], [TokenType.DOUBLE_STAR, TokenType.STAR, TokenType.EOF])

    def test_parentheses(self):
        tokens = tokenize("()")
        self.assertEqual([t.type for t in tokens], [TokenType.LPAREN, TokenType.RPAREN, TokenType.EOF])
...
class TestTokenizeCombinedExpressions(unittest.TestCase):
    ...
    def test_minus_before_number_is_two_separate_tokens(self):
        ...
        tokens = tokenize("-5")
        self.assertEqual([t.type for t in tokens], [TokenType.MINUS, TokenType.NUMBER, TokenType.EOF])
